...
4. `H-01 (P0)` пустой `SPAM_REGEX` отключает только настраиваемую regex-проверку.
5. `H-02 (P0)` невалидный regex не валит процесс и не отключает встроенные эвристики.
6. `H-03 (P1)` regex работает case-insensitive с Unicode-текстом.
